///
//  Generated code. Do not modify.
//  source: common/primitive_messages.proto
//
// @dart = 2.12
// ignore_for_file: annotate_overrides,camel_case_types,unnecessary_const,non_constant_identifier_names,library_prefixes,unused_import,unused_shown_name,return_of_invalid_type,unnecessary_this,prefer_final_fields,deprecated_member_use_from_same_package

import 'dart:core' as $core;
import 'dart:convert' as $convert;
import 'dart:typed_data' as $typed_data;
@$core.Deprecated('Use stringMessageDescriptor instead')
const StringMessage$json = const {
  '1': 'StringMessage',
  '2': const [
    const {'1': 'value', '3': 1, '4': 1, '5': 9, '10': 'value'},
  ],
};

/// Descriptor for `StringMessage`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List stringMessageDescriptor = $convert.base64Decode('Cg1TdHJpbmdNZXNzYWdlEhQKBXZhbHVlGAEgASgJUgV2YWx1ZQ==');
@$core.Deprecated('Use int64MessageDescriptor instead')
const Int64Message$json = const {
  '1': 'Int64Message',
  '2': const [
    const {'1': 'value', '3': 1, '4': 1, '5': 3, '10': 'value'},
  ],
};

/// Descriptor for `Int64Message`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List int64MessageDescriptor = $convert.base64Decode('CgxJbnQ2NE1lc3NhZ2USFAoFdmFsdWUYASABKANSBXZhbHVl');
@$core.Deprecated('Use byteArrayMessageDescriptor instead')
const ByteArrayMessage$json = const {
  '1': 'ByteArrayMessage',
  '2': const [
    const {'1': 'value', '3': 1, '4': 1, '5': 12, '10': 'value'},
  ],
};

/// Descriptor for `ByteArrayMessage`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List byteArrayMessageDescriptor = $convert.base64Decode('ChBCeXRlQXJyYXlNZXNzYWdlEhQKBXZhbHVlGAEgASgMUgV2YWx1ZQ==');
